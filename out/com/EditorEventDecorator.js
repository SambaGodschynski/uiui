...
class EditorEventDecorator {
    ...
    onPlayerStateChanged(state, oldState) {
        if (state === Player_1.PlayerState.Stopped) {
            this.removeDecorationsOnAllSheetEditors();
        }
        const needToUpdateSheetInfo = state === Player_1.PlayerState.StartPlaying
            || (oldState === Player_1.PlayerState.ConnectingToVst && state === Player_1.PlayerState.ConnectedToVst);
